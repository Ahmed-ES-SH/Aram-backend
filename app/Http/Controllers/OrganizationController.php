    ...
    public function getLocations()
    {
        try {
            ...
            $locations = $orgs->getCollection()->transform(function ($org) {
                $decodedLocation = json_decode($org->location); // فك ترميز JSON بشكل صحيح
                return [
                    'id' => $org->id,
                    'address' => $decodedLocation->address ?? null, // إعادة العنوان فقط
                ];
            });

            // تصفية النتائج لإزالة العناوين المكررة
            $uniqueLocations = $locations->unique('address');

            // إعادة النتائج مع بيانات التصفية
            return response()->json([
                'data' => $uniqueLocations->values()->all(), // إعادة العناوين كأرقام متسلسلة
                'pagination' => [
                    'total' => $orgs->total(),
                    'per_page' => $orgs->perPage(),
                    'current_page' => $orgs->currentPage(),
