...
class VisitController extends Controller
{
    public function store(Request $request)
    {
        ...
        $ipExists = Visit::where('user_id', $user->id)
    ...
    public function getUsersWithvisitorsCount()
    {
        try {
            ...
            $users = User::select('id', 'image', 'name', 'user_code')
                ->with('visitors')
                ->withCount('visitors')
                ->orderByDesc('visitors_count') // ترتيب تنازلي حسب عدد العمليات
    ...
    public function getVisitsByUserId($userId)
    {
        try {
            ...
            $purchases = Visit::where('user_id', $userId) // البحث باستخدام id المستخدم
                ->paginate(20); // عدد العناصر في كل صفحة
    ...
    {
        $visits = Visit::where('user_id', $userId)->count();
