...
class ImageService
{
     ...
     * @return void
     */
    public function ImageUploader(Request $request, $user, string $storagePath = 'images/users')
    {
        if ($request->hasFile('image') || $request->hasFile('icon')) {
            // -------------------------
            // تحديد الملف المراد رفعه
            // -------------------------
            $imageFile = $request->hasFile('image') ? $request->file('image') : $request->file('icon');

            // -------------------------
            // تحديد اسم العمود لتخزين الرابط
            // -------------------------
            $columnName = $request->hasFile('image') ? 'image' : 'icon';
            ...
            $user->{$columnName} = url('/') . '/' . 'public/' . $storagePath . '/' . $filename;
            $user->save();
        }
    }
